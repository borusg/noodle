...
class Noodle
  ...
  class Node
    ...
    ATTRIBUTES = %i[name id _id facts params].freeze
      ...
      Noodle::Option.option(record.params['ilk'], 'limits').each do |param,limit|
        ...
        when 'String'
          record.errors.add attr, "#{param} is not a(n) #{limit}" unless
            value[param].nil? || value[param].class.to_s.downcase.match?(/#{limit}/i)
    ...
    def full
      ...
      r << 'Name:   ' + @name
      r << 'Params: '
      r << @params.map { |term, value| "  #{term}=#{value}" }
      r << 'Facts:  '
      r << @facts.map { |term, value| "  #{term}=#{value}" }
      r.join("\n")
    end
