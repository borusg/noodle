...
class Noodle
  class Option
    ...
    def self.refresh
      ...
      # TODO: Does this need DeepDup::deep_dup? Probably not because Hashie?
      @@options['default'] = Hashie::Mash.new(@@builtin_options)
      ...
      Noodle::Search.new(Noodle::NodeRepository.repository).equals('ilk', 'option').go.results.each do |option_hash|
        # NOTE: This is the only time we should reference .params! This
        # is because the options are all Noodle *params* of the
        # ilk=option entry
